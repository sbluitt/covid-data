-- Covid data queries
-- Queries to be used in Tableau are numbered below.
--
...
-- Exluding 'World, European Union, and Internationl' in order to stay consistent with being part of Europe.

SELECT continent, MAX(CAST(total_deaths AS FLOAT)) as total_death_count
FROM covid_deaths
Where continent != "" AND location not in ('World', 'European Union', 'Internationl')
-- WHERE location = "United States"
GROUP BY continent
order by total_death_count desc;

-- 3.
-- Looking at countries with highest infection rate compared to population using MAX function.
...
-- 4.
-- Looking at countries with highest infection rate compared to population using MAX function and added date.
...
--
-- Created quiery examples to be used in Tableau below.
--
...
-- Looking at countries infection rates compared to populations.
...
Select location, continent, SUM(new_deaths) as total_death_count
From covid_deaths
Where continent != "" AND location not in ('World', 'European Union', 'Internationl')
